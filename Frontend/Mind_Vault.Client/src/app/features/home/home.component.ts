...
@Component({
  ...
  template: `
    <section class="grid gap-10 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
      <div class="space-y-6">
        <span class="inline-flex items-center gap-2 rounded-full border border-vault-gold/30 bg-white/70 px-4 py-2 text-sm font-semibold text-vault-ink shadow-sm backdrop-blur dark:border-vault-gold/20 dark:bg-slate-800/80 dark:text-vault-paper">
        ...
        <div class="space-y-4">
          ...
          <h1 class="max-w-3xl font-display text-5xl leading-tight text-vault-ink dark:text-vault-paper md:text-6xl">
        ...
        <div class="flex flex-col gap-3 sm:flex-row">
          @if (authService.isAuthenticated()) {
            ...
            <a
              routerLink="/login"
              class="inline-flex items-center justify-center gap-2 rounded-full border border-vault-ink/20 bg-white/70 px-6 py-3 text-sm font-semibold text-vault-ink transition hover:-translate-y-0.5 hover:border-vault-teal hover:text-vault-teal dark:border-slate-600 dark:bg-slate-900 dark:text-vault-paper"
            >
      ...
      <aside class="relative overflow-hidden rounded-[2rem] border border-white/70 bg-white/80 p-6 shadow-card backdrop-blur dark:border-slate-700 dark:bg-slate-900/80">
        ...
        <div class="space-y-4">
          ...
          <div class="grid gap-4">
            <article class="rounded-3xl bg-vault-paper p-5 dark:bg-slate-800/80">
              <div class="mb-3 inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-vault-teal/10 text-vault-teal">
                <i class="fa-solid fa-book"></i>
              </div>
              <h2 class="font-display text-2xl text-vault-ink dark:text-vault-paper">Books worth keeping</h2>
            ...
            <article class="rounded-3xl bg-vault-paper p-5 dark:bg-slate-800/80">
              ...
              <h2 class="font-display text-2xl text-vault-ink dark:text-vault-paper">Quotes that stay with you</h2>
              <p class="mt-2 text-sm leading-7 text-slate-700 dark:text-slate-200">
